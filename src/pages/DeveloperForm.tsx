...
export default function DeveloperForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError('')
    if (!form.name || !form.yearsOfExperience) {
      setError('Please fill in all required fields.')
      return
    }
    setSubmitting(true)
    try {
      const res = await fetch(`${API}/developer-form`, {
        method: 'POST',
      ...
      navigate('/developer/agreement', { state: { form } })
    } catch {
      ...
      <div className="form-page" style={{ padding: '120px 40px 80px', maxWidth: 640, margin: '0 auto' }}>
        <motion.div className="form-page-inner" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 style={{ fontFamily: "'Urbanist', sans-serif", fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, lineHeight: 1.1, letterSpacing: '-0.03em', margin: '0 0 48px' }}>
            Developer
            <br />
            <span style={{ color: '#C6FF00' }}>Registration</span>
          </h1>
                  ...
                  {SPECIALTIES.map((s) => (
                    <button
                      ...
                      style={{
                        ...
                        padding: '8px 16px', border: `1px solid ${form.specialties.includes(s) ? '#C6FF00' : '#222'}`,
                        background: form.specialties.includes(s) ? 'rgba(198,255,0,0.08)' : 'transparent',
                        color: form.specialties.includes(s) ? '#C6FF00' : '#4A4A4A',
